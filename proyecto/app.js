...
app.controller('MainController', function($scope, $http, $window, $timeout) {
    ...
    $scope.syncDataFromServer = function() {
        ...
        $http.get($scope.apiUrl + "?action=getData").then(res => {
            if(res.data.success) {
                ...
                // Disparadores visuales dependiendo de la pestaña activa
    ...
    $scope.guardarObra = function() {
        ...
        $scope.loading = true;
        $http.post($scope.apiUrl, JSON.stringify({ action: 'saveObra', payload: $scope.obraActual })).then(() => { $scope.obraActual={}; $scope.syncDataFromServer(); });
    };
    ...
    $scope.guardarGasto = function() {
        if(!$scope.esAdmin) return;
        $scope.loading = true; 
        let payload = angular.copy($scope.gastoActual); 
        delete payload.OBRA_OBJ;
    ...
    // --- REPORTES Y ANALÍTICAS (Pestaña 3) ---
    ...
    $scope.procesarGraficosPresupuestos = function() {
        ...
        let labelsObras = $scope.presupuestos.map(p => p.OBRA);
        let dataPresupuestos = $scope.presupuestos.map(p => p.PRESUPUESTO_MONTO);
        let dataInversion = $scope.presupuestos.map(p => p.TOTAL_INVERSION);

        // KPI Acumulados Globales
        $scope.kpiPresupuestos = {
            totalPresupuesto: dataPresupuestos.reduce((a, b) => a + b, 0),
            totalInversion: dataInversion.reduce((a, b) => a + b, 0),
            margenGlobal: dataPresupuestos.reduce((a, b) => a + b, 0) - dataInversion.reduce((a, b) => a + b, 0)
        };

        // 1. Gráfica Comparativa (Presupuesto vs Inversión)
        ...
        chartInstInversion = new Chart(document.getElementById('chartInversion'), {
            ...
            data: {
                ...
                datasets: [
                    ...
                    { label: 'Total Inversión ($)', data: dataInversion, backgroundColor: '#f59e0b', borderRadius: 4 }
                ]
            },
            options: { responsive: true, maintainAspectRatio: false }
        });

        // 2. Gráfica de Desglose Global (Doughnut)
        let totalManoObra = $scope.presupuestos.reduce((acc, p) => acc + p.MANO_OBRA_MONTO, 0);
        let totalEquipos = $scope.presupuestos.reduce((acc, p) => acc + p.EQUIPOS_MONTO, 0);
        let totalMateriales = $scope.presupuestos.reduce((acc, p) => acc + p.MATERIALES_VARIOS_MONTO, 0);
        let totalLamina = $scope.presupuestos.reduce((acc, p) => acc + p.LAMINA_MONTO, 0);

        if(chartInstDesglose) chartInstDesglose.destroy();
        chartInstDesglose = new Chart(document.getElementById('chartDesglose'), {
            type: 'doughnut',
            data: {
                labels: ['Mano de Obra', 'Equipos', 'Materiales Varios', 'Lámina'],
                datasets: [{
                    data: [totalManoObra, totalEquipos, totalMateriales, totalLamina],
                    backgroundColor: ['#10b981', '#8b5cf6', '#ec4899', '#f97316'],
                    borderWidth: 0,
                    hoverOffset: 4
                }]
            },
